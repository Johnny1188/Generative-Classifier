...
class ClassifierHeadBlock(nn.Module):
    def __init__(self, layer_shapes=(32,64,32,10), input_dim=1024):
...
class ClassifierCNNBlock(nn.Module):
    def __init__(self, cnn_layers=(16,32,64), input_dims=(28,28,1)):
        assert len(input_dims) == 3
        assert len(cnn_layers) > 2
        super().__init__()

        cnn_stack_build = [
            nn.Conv2d(input_dims[-1], cnn_layers[0], kernel_size=3, stride=1),
            nn.ReLU(),
        ...
        for cnn_l_id in range(1,len(cnn_layers)):
            ...
            if cnn_l_id != len(cnn_layers) - 1:
                cnn_stack_build.append(nn.MaxPool2d(kernel_size=2, stride=2))
